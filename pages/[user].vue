...
const fetchRulingsAndTags = async () => {
  try {
    ...
    // Fetch data from baywidi_urteile where gericht_abk matches the courtAbk
    const { data, error } = await supabase
      ...
      .eq('gerichte.gericht_abk', courtAbk); // Filter by court abbreviation

    if (error) {
      errorMessage.value = 'Error fetching data from the database.';
      console.error('Supabase Error:', error);
      return;
    }

    // Update rulings with the fetched data
    rulings.value = data;
